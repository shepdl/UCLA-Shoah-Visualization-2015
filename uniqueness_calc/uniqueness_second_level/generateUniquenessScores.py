# This is to calculate the SECOND LEVEL parents based uniqueness scores for all
...
# Read second level parent base probabilities.
query1 = """
SELECT * 
FROM secondLevelBaseProbabilities2015
"""
...
# Get second level parent climbed to from the keyword of the testimony segmentile 
...
query2 = """
SELECT distinct SecondLevelParentID, SegmentNumber 
			...
			if not row:
				...
				if index[curr-1]!=index[prev-1]:
					...
					if curr_segnum == row["SegmentNumber"]:
						flag = 1
						seg_parent_counts[row["SecondLevelParentID"]] = 1
					flag = 1
